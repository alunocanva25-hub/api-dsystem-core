...
## V1.0.1.6 — Compatibilidade Financeira GO

Patch focado em `/api/transactions`, aliases financeiros e resumo de receitas/despesas/saldo para o DS STUDIO GO.


## V1.0.1.8 — Preservar códigos do Studio

Clientes sincronizados pelo DSYSTEM STUDIO preservam códigos como STR0000 em external_id/codigo/sigla/client_code.


## V1.0.1.9 — Conciliar códigos Studio

Evita duplicar clientes antigos `CLI-X` quando o Studio volta a enviar a sigla oficial `STRxxxx`. O sync migra/concilia de forma conservadora, reponta referências de Agenda/Financeiro e preserva o ID legado em `raw_payload`. Inclui `POST /api/admin/reconcile-client-codes` para conciliação administrativa dos registros já existentes.
