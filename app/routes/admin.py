...
from app.services.sync_service import reconcile_existing_customer_codes
...
@router.post("/reconcile-client-codes")
def admin_reconcile_client_codes(company_id: int | None = None, db: Session = Depends(get_db), current_user=Depends(require_master_or_admin)):
    target_company = company_id or current_user.company_id
    stats = reconcile_existing_customer_codes(db, company_id=target_company)
    write_audit(db, company_id=target_company, user_id=current_user.id, module_code="core", action="RECONCILE_CLIENT_CODES", description=f"Conciliação CLI-X -> STRxxxx: {stats}")
    return {"ok": True, "company_id": target_company, **stats}
