...
def _client_out(c: Customer) -> dict[str, Any]:
    ...
    try:
        ...
        raw = {}
    return {
        # Para clientes vindos do Studio, id é interno da API; o código funcional é STR0000.
        # Mantemos aliases para GO/Studio não substituírem por CLI-X.
        "id": c.id,
        "api_id": c.id,
        "external_id": c.external_id,
        "codigo": raw.get("codigo") or raw.get("sigla") or raw.get("client_code") or c.external_id,
        "sigla": raw.get("sigla") or raw.get("codigo") or raw.get("client_code") or c.external_id,
        "client_code": raw.get("client_code") or raw.get("codigo") or raw.get("sigla") or c.external_id,
        "customer_code": raw.get("customer_code") or raw.get("codigo") or raw.get("sigla") or c.external_id,
        "studio_code": raw.get("studio_code") or raw.get("codigo") or raw.get("sigla") or c.external_id,
...
def _appointment_out(a: Appointment) -> dict[str, Any]:
    ...
    return {
        ...
        "customer_external_id": a.customer_external_id,
...
def _apply_pull_sources(q, model, *, only_go_mobile: bool, sources: str | None):
    source_list = [part.strip() for part in str(sources or "").split(",") if part.strip()]
    if source_list:
        return q.filter(model.sync_source.in_(source_list))
    if only_go_mobile:
        return q.filter(model.sync_source == "go_mobile")
    return q


@router.get("/api/studio/pull/clients")
def studio_pull_clients(only_go_mobile: bool = True, sources: str | None = None, limit: int = 500, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    q = db.query(Customer).filter(Customer.company_id == current_user.company_id)
    q = _apply_pull_sources(q, Customer, only_go_mobile=only_go_mobile, sources=sources)
    return [_client_out(x) for x in q.order_by(Customer.updated_at.desc()).limit(limit).all()]


@router.get("/api/studio/pull/appointments")
def studio_pull_appointments(only_go_mobile: bool = True, sources: str | None = None, limit: int = 500, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    q = db.query(Appointment).filter(Appointment.company_id == current_user.company_id)
    q = _apply_pull_sources(q, Appointment, only_go_mobile=only_go_mobile, sources=sources)
...
def studio_pull_transactions(only_go_mobile: bool = True, sources: str | None = None, limit: int = 500, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    q = db.query(TransactionRecord).filter(TransactionRecord.company_id == current_user.company_id)
    q = _apply_pull_sources(q, TransactionRecord, only_go_mobile=only_go_mobile, sources=sources)
...
def go_compatibility_map():
    return {
        ...
        "sources": ["desktop_sync", "go_mobile", "online_booking", "api_local"],
        "notes": "Camada bridge compatível com Studio/DS Go e Agenda Online CORE; pull por source evita duplicação entre desktop, mobile e online.",
    }
