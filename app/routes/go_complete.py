...
def create_client_fallback(payload: dict[str, Any], db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    item = {**payload, "source": payload.get("source") or "api_local"}
    if not item.get("external_id"):
        item["external_id"] = item.get("codigo") or item.get("sigla") or item.get("client_code") or item.get("customer_code") or item.get("studio_code") or f"api_client_{int(datetime.now(timezone.utc).timestamp()*1000)}"
    code = item.get("external_id")
    if code:
        item.setdefault("codigo", code)
        item.setdefault("sigla", code)
        item.setdefault("client_code", code)
        item.setdefault("customer_code", code)
        item.setdefault("studio_code", code)
